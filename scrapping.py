...
# 1. Fetch token securely from Google Colab Vault
try:
    API_KEY = userdata.get('BRIGHTDATA_API_KEY')
    print("✅ Bright Data credential initialized securely.")
except userdata.SecretNotFoundError:
    raise ValueError("❌ Action required: Add 'BRIGHTDATA_API_KEY' to your Colab Secrets sidebar!")
...
if __name__ == "__main__":
    ...
    if scraped_posts and isinstance(scraped_posts, list):
        for post in scraped_posts:
            ...
            # Use a list to maintain order and a set to prevent duplicate comments for THIS specific post
            filtered_comments = []
            seen_comments = set()
            ...
            if isinstance(raw_comments, list):
                for item in raw_comments:
                    ...
                    if comment_text and isinstance(comment_text, str):
                        # Clean/strip whitespace to ensure exact matches are caught properly
                        cleaned_comment = comment_text.strip()
                        
                        # Filter text items containing strictly greater than 25 words AND check for duplicates
                        if len(cleaned_comment.split()) > 25 and cleaned_comment not in seen_comments:
                            filtered_comments.append(cleaned_comment)
                            seen_comments.add(cleaned_comment) # Mark as seen
        ...
        print("\n🏆 Processing complete! Saving data and initiating download...")
        
        # Define filename
        filename = "scraped_results.json"
        
        # Save payload to a file locally in Colab environment
        ...
        print("\n🛑 Execution paused. No posts collected. Check trigger log details printed above.")
